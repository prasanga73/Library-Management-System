...
def add_user():
    def submit():
        id,name, email, password, mobile, address =entry_id.get(), entry_name.get(), entry_email.get(), entry_pass.get(), entry_mobile.get(), entry_address.get()
    ...
    Label(add_win, text="Enter User Id:").pack(pady=5)
    entry_id = Entry(add_win)
...
def add_issue():
    ...
    Label(issue_win, text="Enter Book Id:").pack(pady=5)
    entry_book_id = Entry(issue_win)
...
def add_author():
    ...
    Label(author_win, text="Enter Author Id:").pack(pady=5)
    entry_authorid = Entry(author_win)
    entry_authorid.pack(pady=5)
...
def add_book():
    def submit():
        bookid, title, author_id, isbn, num,price = entry_bookid.get(), entry_title.get(), entry_author_id.get(), entry_isbn.get(), entry_num.get(),entry_price.get()
        if title and author_id and isbn:
            cursor.execute("INSERT INTO books (book_id,book_name,author_id,cat_id,book_no,book_price) VALUES (%s,%s, %s, %s, %s,%s)", 
                          (bookid,title, author_id, isbn,num,price ))
            db_connection.commit()
